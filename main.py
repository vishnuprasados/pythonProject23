...
class CurrencyConverterUI(tk.Tk):
    def __init__(self, converter):
        ...
        # Label
        self.intro_label = Label(self, text='CURRENCY CONVERTER', fg='red', bg='black', relief=tk.RAISED,
                                 borderwidth=3)
        self.intro_label.config(font=("Courier", 15, 'bold'))
        # self.date_label = Label(self, text= f"1 Indian Rupee equals = {self.currency_converter.convert('INR','USD', 100)} USD /n Date : {self.currency_converter.data['date']}", relief=tk.GROOVE, borderwidth=5 )

        # placing labels
        self.intro_label.place(x=120, y=10)
        ...
        self.to_currency_dropdown.place(x=270, y=120)
        # self.converted_amount_field.place(x = 346, y = 150)
        self.converted_amount_field_label.place(x=270, y=152)

        # convert button
        self.convert_button = Button(self, text="Convert", fg='black', bg='grey', command=self.perform, height=2,
                                     width=7)
